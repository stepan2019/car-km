...
class Car {
    ...
    public $username = "carkm_Fouad";
    public $password = "1?U}=aQ5U27M";
    public $dbname = "carkm_km";
    public $connectdb;
    ...
    // password forgot
    public function getUserFromEmail($email, $type) {
        if($type == "user")
    ...
    public function login_dealer($email, $password) {
    ...
    public function blockCheck($email, $type) {
        if($type == "user")
            $query = "select * from user where email = '$email'";
        else
            $query = "select * from dealer where email = '$email'";
    ...
    // add vehicle
    public function add_vehicle($user_id, $type, $plate, $vin, $make, $model, $year, $km, $date, $crash, $front, $back, $lefty, $righty, $total) {
        $query = "select id from vehicle where vin = '$vin'";
        $result = $this->connectdb->query($query);
        $existVehicle = $result->num_rows;

        if($existVehicle > 0) {
            $query = "update vehicle set plate = '$plate', make = '$make', model = '$model', year = '$year', user_id = '$user_id', type = '$type' where vin = '$vin'";
        } else {
            $query = "insert into vehicle(plate, vin, make, model, year, user_id, type) value('$plate', '$vin', '$make', '$model', '$year', '$user_id', '$type')";
        }
    ...
    public function get_vehicle_by_plate($plate) {
        $query = " select * from vehicle where plate = '$plate'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function get_vehicle_by_vin($plate) {
        $query = " select * from vehicle where vin = '$plate'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function get_vehicle_km_by_car_id($car_id) {
        $query = " select * from vehicle_km where car_id = '$car_id' order by add_date DESC, id DESC";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function get_last_vehicle_km_by_car_id($car_id) {
    ...
    // get user address
    public function getAddress($email, $type) {
        if($type == "user")
            $query = "select * from user where email = '$email'";
        else
            $query = "select * from dealer where email = '$email'";
    ...
    // add make
    public function add_make($name) {
        $query = "insert into make(name) values('$name')";
        $result = $this->connectdb->query($query);
        return $result;
    }
    public function add_provin($name) {
    ...
    public function getMaekIdByName($name) {
        $query = "select * from make where name = '$name'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function existMake($new_make) {
    ...
    public function existProvin($new_provin) {
        $query = "select * from province where name = '$new_provin'";
        $result = $this->connectdb->query($query);
        $exist = $result->num_rows;
    ...
    public function deleteMake($makelist) {
    ...
    public function deleteProvin($provinlist) {
        for($i = 0; $i < count($provinlist); $i++) {
            $name = $provinlist[$i];
    ...
    // add model
    public function add_model($name, $selectedMakeName) {
    ...
    public function getModelListFromMake($selectedMake) {
    ...
    public function existModel($new_model, $selectedMakeName) {
        $result = $this->getMaekIdByName($selectedMakeName);
        $obj = $result->fetch_assoc();
        $dep = $obj['id'];
    ...
    public function deleteModel($modellist) {
    ...
    // admin
    public function admin_check($username, $password) {
    ...
    public function updateDealer($id, $name, $address, $email, $phone, $company, $website) {
    ...
    // user block
    public function userBlock($id, $isChecked) {
        $query = "update user set block = '$isChecked' where id = '$id'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function dealerBlock($id, $isChecked) {
        $query = "update dealer set block = '$isChecked' where id = '$id'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function getBlockList($type) {
        if($type == "user")
    ...
    public function getCrashedByCarId($car_id) {
    ...
    public function deleteCrashByCarId($car_id) {
        $query = "delete from vehicle_crash where car_id = '$car_id'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function deleteKmByCarId($car_id) {
    ...
    public function updateVehicle($id, $plate, $make, $model, $year) {
        $query = "update vehicle set plate = '$plate', make = '$make', model = '$model', year = '$year' where id = '$id'";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function updateVehicleCrash($car_id, $crash, $front, $back, $lefty, $righty, $total) {
        $crashedStateResult = $this->getCrashedByCarId($car_id);
        $crashedState = $crashedStateResult->fetch_assoc();
        if($crashedState['crash'] == "yes") {
            $crash = "yes";
    ...
    public function updateVehicleKm($car_id, $km, $date) {
        $logic = "";
        $result = $this->get_vehicle_km_by_car_id($car_id);
        $number = $result->num_rows;
        if($number == 0) {
    ...
    // price
    public function add_price($download_price) {
    ...
    //coupon
    public function add_coupon($code, $allow_usage) {
        $query = "insert into coupon(code, allow_usage) values('$code','$allow_usage')";
        $result = $this->connectdb->query($query);
        return $result;
    }

    public function update_coupon($code, $amount) {
        if(!$amount){
            return 0;
        }
    ...
    public function edit_coupon($code, $allow_usage) {
    ...
    //coupon use history
    public function add_coupon_history($code, $amount,$vin,$payer_email) {
    ...
    public function deleteCouponHistory($id) {
    ...
    public function update_profile($username, $email) {
    ...
    // user password
    public function changePassword($id, $type, $new_password) {
        if($type == "user")
    ...
    // about content
    public function add_about_content($content, $content2) {
    ...
    // index content
    public function add_content_content($content1, $content2) {
    ...
    public function add_content($content1, $content2) {
        $query = "insert into content(content1, content2) values('$content1', '$content2')";
        $result = $this->connectdb->query($query);
        return $result;
    }
    ...
    public function add_footer($content1, $content2) {
    ...
    // information content
    public function add_information_content($content) {
    ...
    // image upload
    public function imageUpload($fileName, $bannerText, $position, $width, $height) {
    ...
    public function loadBanner($position) {
    ...
    public function getHomescreenById($id) {
    ...
    // update homescreen
    public function updateHomescreen($id, $file_name, $title, $position, $width, $height) {
    ...
    // add Ads Banner
    public function addAdsBanner($fileName, $bannerText, $link, $position, $start_date, $end_date) {
        $activate = "yes";
    ...
    // add Ads Banner
    public function updateAdsBanner($id, $fileName, $bannerText, $link, $position, $start_date, $end_date) {
    ...
    public function loadAdsBanner($position) {
        $today = date('Y-m-d');
    ...
    // get activate state
    public function getActivateState($id) {
        $query = "select * from adsbanner where id = '$id'";
        $result = $this->connectdb->query($query);
        $obj = $result->fetch_assoc();
        return $obj['activate'];
    ...
    // paypal payment
    public function putPaymentResult($email, $itemAmount) {
