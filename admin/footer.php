...
$result = $config->getFooterByCode($lang_id);
...
if (isset($_POST['add_content'])) {
    ...
    $is_add = $config->add_footer($content1, $content2, $lang_id);
    if ($is_add) {

        $result = $config->getFooterByCode($lang_id);
        $content = $result->fetch_assoc();
        echo "<script>window.location = 'home.php?query=footer&lang_id=" . $lang_id . "';</script>";
    } else {
        ...
        <div class="right_div" align="right" style="margin: 8px 10px 0 0;">
            ...
            <select name="lang" onchange="doSel(this);">
                ...
                while ($row = $languages->fetch_assoc()) {
                    ?>
                    <option value="location.href='home.php?query=footer&lang_id=<?php echo $row['code']; ?>'"
