		...
		if(isset($_POST["submit"]) && !empty($_FILES["file"]["name"])) {
		    ...
		    if(in_array($fileType, $allowTypes)) {
		        if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
		            $insert = $config->imageUpload($fileName, $bannerText, $position, $width, $height);
		            if($insert) {
...
<form method="post" enctype="multipart/form-data">
	...
	<div class="col-md-12 mt-5"><input type="submit" name="submit" value="Add New Home Screen" class="btn btn-primary btn-custom"></div>
</form>