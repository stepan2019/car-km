...
if (isset($_POST['add_about'])) {
    ...
    $is_success = $config->add_about_content($content, $content2, $lang_id);
    if ($is_success) {
        $result = $config->getAboutContentByCode($lang_id);
        $about = $result->fetch_assoc();
        echo "<script>window.location = 'home.php?query=about&lang_id='" . $lang_id . ";</script>";
    } else {
    ...
    <form method="post">
        ...
        <div class="right_div" align="right" style="margin: 8px 10px 0 0;">
            ...
            <select name="lang" onchange="doSel(this);">
                ...
                while ($row = $languages->fetch_assoc()) {
                    ?>
                    <option value="location.href='home.php?query=about&lang_id=<?php echo $row['code']; ?>'"
        ...
        <textarea id="ckeditor" class="ckeditor" name="content"
            ...
            <textarea id="ckeditor" class="ckeditor" name="content2"
                      style="min-height:250px;"><?php echo $about['content2'] ?></textarea>
